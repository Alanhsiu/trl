...
import os
import numpy as np
from dpo_eval import get_reward_mos, get_reward_asr, eval_dpo_mos, eval_dpo_asr
import json
...
def generate_output_batch(
    ...
    if valid_audio_paths:
        ...
        for audio_path in valid_audio_paths:
            reward = get_reward_asr(file_path=audio_path, asr_model=asr_model)
            reward_list.append(reward)
            
    print(f"Time taken for calculate score: {time.time() - start_predict_time} seconds")
...
def process_data_batch(
    ...
    strategy: str = "top_bottom_percent"  # Options: "top_bottom_percent" or "above_below_average" or "above_prev_eval"
...
gradient_accumulation_steps = 1 # Training: gradient accumulation steps (original: 1)
...
def evaluate_asr_model(eval_type, eval_data_len, eval_indices):
        
        # Evaluate the model
        original_model_metrics_asr, original_model_rewards_asr = eval_dpo_asr(
            nar_model=nar_model,
            ar_tokenizer=ar_tokenizer,
            nar_tokenizer=nar_tokenizer,
            trained_model=model,
        ...
        logging.info(f"Original model metrics on {eval_type} set: {original_model_metrics_asr}")
        logging.info(f"Original model rewards on {eval_type} set: {original_model_rewards_asr}")
    
        # Calculate and log ASR scores
        reward_list_asr = [np.mean([r for r in rewards if r is not None]) if any(r is not None for r in rewards) else None
                        for rewards in original_model_rewards_asr]
        logging.info(f"Original model ASR score list on {eval_type} set: {reward_list_asr}")
    
        # Filter and calculate average ASR
        filter_reward_list_asr = [r for r in reward_list_asr if r is not None]
        if filter_reward_list_asr:
            average_asr = np.mean(filter_reward_list_asr)
            logging.info(f"Original model average ASR on {eval_type} set: {average_asr}")
        else:
            logging.info(f"Original model average ASR on {eval_type} set: None")
    
        # Calculate and log weighted reward
        weighted_reward = np.mean(filter_reward_list_asr) if filter_reward_list_asr else None
...
if eval_train:
    prev_eval_avg = evaluate_asr_model(eval_type="training", eval_data_len=eval_train_data_len, eval_indices=eval_train_indices)

if eval_test:
    prev_eval_avg = evaluate_asr_model(eval_type="testing", eval_data_len=eval_test_data_len, eval_indices=eval_test_indices)
...
def evaluate_iteration_model(eval_type, iteration, eval_data_len, eval_indices):
    ...
    trained_model_metrics, trained_model_rewards = eval_dpo_asr(
        nar_model=nar_model,
        ar_tokenizer=ar_tokenizer,
        nar_tokenizer=nar_tokenizer,
        trained_model=model,
    ...
    logging.info(f"EVAL: ASR metrics {eval_type.capitalize()} Set for iteration {iteration}: {trained_model_metrics}")
    logging.info(f"EVAL: ASR score {eval_type.capitalize()} Set for iteration {iteration}: {trained_model_rewards}")
    ...
    weighted_reward = np.mean(filter_reward_list) if filter_reward_list else None
